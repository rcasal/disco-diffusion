...
# def create_perlin_noise(side_y, side_x, octaves=[1, 1, 1, 1], width=2, height=2, grayscale=True):
...
# def regen_perlin(perlin_mode, args):
...
#             if cutout_debug:
#                 TF.to_pil_image(cutouts[0].clamp(0, 1).squeeze(0)).save(f"{self.args.root_path}/cutout_overview0.jpg",quality=99)
...
#                 cutouts.append(cutout)
#             if cutout_debug:
#                 TF.to_pil_image(cutouts[-1].clamp(0, 1).squeeze(0)).save(f"{self.args.root_path}/cutout_InnerCrop.jpg",quality=99)
...
#                 if args.vr_mode:
#                   generate_eye_views(TRANSLATION_SCALE,args.batchFolder,filename,frame_num,midas_model, midas_transform)
#                 continue
#               else:
...
#                 txt = clip_model.encode_text(clip.tokenize(prompt).to(args.device)).float()
...
#                   batch = model_stat["make_cutouts"](TF.to_tensor(img).to(args.device).unsqueeze(0).mul(2).sub(1))
...
#             model_stat["weights"] = torch.tensor(model_stat["weights"], device=args.device)
...
#                 alpha = torch.tensor(args.diffusion.sqrt_alphas_cumprod[cur_t], device=args.device, dtype=torch.float32)
#                 sigma = torch.tensor(args.diffusion.sqrt_one_minus_alphas_cumprod[cur_t], device=args.device, dtype=torch.float32)
#                 cosine_t = args.alpha_sigma_to_t(alpha, sigma)
#                 out = args.secondary_model(x, cosine_t[None].repeat([n])).pred
#                 fac = args.diffusion.sqrt_one_minus_alphas_cumprod[cur_t]
#                 x_in = out * fac + x * (1 - fac)
#                 x_in_grad = torch.zeros_like(x_in)
#               else:
#                 my_t = torch.ones([n], device=args.device, dtype=torch.long) * cur_t
